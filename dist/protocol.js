...
export function decodeListingResponse(response) {
    const message = response.message;
    if (response.status !== "CONTINUE" || !message || message.format.schemaId !== MARKET_SCHEMA_ID || message.format.templateId !== 107 || message.format.version !== 10 || message.format.blockLength !== 0)
