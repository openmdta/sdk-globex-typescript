export function decodeListingEvent(bytes) {
    ...
    const u64 = (value) => {
        if (typeof value !== "string" || !/^(0|[1-9][0-9]*)$/.test(value))
            throw new Error("invalid listing timestamp or message ID");
        const result = BigInt(value);
        if (result > 18446744073709551615n)
            throw new Error("listing timestamp or message ID exceeds u64");
        return result;
    };
    if (!event || typeof event.incarnation !== "string" || typeof event.snapshot !== "boolean" || typeof event.connected !== "boolean" || !Array.isArray(event.blocks))
        throw new Error("invalid listing event");
    const source = event.source;
    if (!source || typeof source.dataset !== "string" || typeof source.key !== "string" || !["RT", "DL", "EOD"].includes(source.quality) || !Array.isArray(source.blocks) || !source.blocks.every((id) => Number.isInteger(id) && Number(id) >= 0 && Number(id) <= 65535))
        throw new Error("invalid listing source");
    return { ...event, blocks: event.blocks.map((block) => {
