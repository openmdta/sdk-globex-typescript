...
export const decodeFeedControl = (response: StandardResponse): FeedWireControl => {
  ...
  if (dataset.next !== body.byteLength) throw new ProtocolError("feed control contains trailing bytes");
  return {
...
export interface FeedWireSnapshotHeader {
  readonly throughMessageId: bigint;
  readonly gaps: readonly { readonly afterMessageId: bigint; readonly throughMessageId: bigint }[];
  readonly dataset: string;
}
...
export const decodeFeedSnapshotHeader = (response: StandardResponse): FeedWireSnapshotHeader => {
  ...
  if (gaps.some(gap => gap.afterMessageId >= gap.throughMessageId)) throw new ProtocolError("invalid feed snapshot gap");
  const dataset = takeVarData(body, view, group.next + group.count * 16);
  if (dataset.next !== body.byteLength) throw new ProtocolError("feed snapshot header contains trailing bytes");
  return {
...
export type CatalogFeedWireControl =
  ...
  | {readonly kind: "snapshot_complete"; readonly cursor: string}
...
export const splitResponseBatch = (response: StandardResponse): readonly StandardResponse[] => {
  ...
  for (let index = 0; index < count; index++) {
    if (offset + 12 > body.byteLength) throw new ProtocolError("truncated response batch");
    const format: SbeFormat = {schemaId: view.getUint16(offset, true), templateId: view.getUint16(offset + 2, true),
  ...
  if (offset !== body.byteLength) throw new ProtocolError("response batch has trailing bytes");
  return messages;
};
