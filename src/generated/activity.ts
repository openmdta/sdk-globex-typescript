// Generated from openmdta-core activity types. Do not edit.
...
function valid(node: Schema, value: unknown): boolean {
  if (node.$ref) {
  ...
  if (alternatives) return alternatives.some(variant => valid(variant, value));
  if (Array.isArray(node.type)) return node.type.some(type => valid({ ...node, type }, value));
  switch (node.type) {
...
export function decodeStreamMetadataJson(bytes: Uint8Array): StreamMetadata {
  if (bytes.byteLength > MAX_METADATA_BYTES) throw new Error("Stream metadata exceeds 128 KiB");
  const value: unknown = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes));
  if (!valid(schema, value)) throw new Error("invalid Stream metadata response");
